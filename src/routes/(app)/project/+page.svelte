	...
	const filteredProjects = $derived(
		projectsData.all
			.filter((p) => {
				...
				const localizedDesc =
					typeof messageFunc === 'function'
						? (messageFunc as (p: object, o: object) => string)(
								{},
								{ locale: i18n.current }
							).toLowerCase()
						: '';
					...
					>
						...
						<Card.Header class="space-y-2">
							<div class="flex justify-between items-start gap-2">
								<Card.Title
									class="text-xl font-black leading-tight group-hover:text-primary transition-colors"
						...
						<Card.Content class="grow space-y-4">
							...
							<div class="flex flex-wrap gap-1.5">
								{#each project.tags as tag}
									<Badge
										variant="secondary"
										class="text-[9px] font-black uppercase px-2 py-0.5 bg-secondary/50 hover:bg-secondary transition-colors"
									>
										{tag}
									</Badge>
								{/each}
						...
						<Card.Footer class="flex flex-col gap-2 pt-4 border-t bg-muted/20">
							<Button
								...
								class="w-full font-black uppercase tracking-widest text-xs shadow-sm hover:shadow-md transition-all"
							>
		...
		{:else}
			<div class="col-span-full py-20 text-center space-y-4" in:fade>
				...
				<p class="text-muted-foreground font-medium">
					{m.no_project_desc({}, { locale: i18n.current })}
				</p>
				<Button
					variant="link"
					onclick={() => {
