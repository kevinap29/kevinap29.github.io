...
<svelte:head>
	...
	<meta name="description" content={metadata?.problem || project.name} />
</svelte:head>

<div class="max-w-5xl mx-auto px-4 py-8 space-y-8" in:fade={{ duration: 300 }}>
	<!-- Breadcrumb -->
	<Breadcrumb.Root>
		<Breadcrumb.List>
			<Breadcrumb.Item>
	...
	<section class="grid grid-cols-1 lg:grid-cols-3 gap-12 pt-4">
		<div class="lg:col-span-2 space-y-6">
			...
			<div class="aspect-video rounded-3xl overflow-hidden border border-border/50 shadow-2xl relative group">
				<img 
		...
		<div class="space-y-8">
			<div class="p-6 rounded-3xl bg-muted/30 border border-border/50 space-y-6 backdrop-blur-sm">
				<div class="space-y-4">
					<h3 class="text-xs font-black uppercase tracking-widest text-primary">Project Info</h3>
					<div class="space-y-4">
						<div class="flex items-center gap-3">
							<div class="p-2 rounded-xl bg-background shadow-sm">
				...
				<div class="flex flex-col gap-3">
					{#if project.url !== '#'}
						<Button href={project.url} target="_blank" class="w-full font-black uppercase tracking-widest rounded-2xl h-12 shadow-lg hover:shadow-primary/20 transition-all">
							<ExternalLink class="size-4 mr-2" />
							Live Demo
						</Button>
					{/if}
					<Button variant="outline" href="/project" class="w-full font-black uppercase tracking-widest rounded-2xl h-12">
	...
	<section class="grid grid-cols-1 lg:grid-cols-3 gap-12">
		<div class="lg:col-span-2 prose prose-stone dark:prose-invert prose-headings:font-black prose-headings:tracking-tighter prose-p:text-muted-foreground prose-p:leading-relaxed max-w-none">
			{#if content}
				{@html content}
			{:else}
				<div class="py-20 text-center border-2 border-dashed rounded-3xl space-y-4">
		...
		<div class="hidden lg:block space-y-6">
			{#if metadata?.problem}
				<div class="p-6 rounded-3xl bg-primary/5 border border-primary/10 space-y-3">
