	...
	<div class="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
		...
		<div class="lg:col-span-2 space-y-6">
			<Reveal delay={500} x={-20}>
				<Card.Root class="rounded-none border-primary/5 bg-muted/20 hover:bg-muted/30 transition-colors">
					<Card.Header class="flex flex-row items-center gap-4 space-y-0">
						<div class="size-12 rounded-none bg-background border border-primary/10 flex items-center justify-center text-primary">
							<Mail class="size-6" />
						</div>
						<div>
							<Card.Title class="text-xs font-black uppercase tracking-widest text-muted-foreground">Email</Card.Title>
							<Card.Description class="text-lg font-bold text-foreground break-all">{email}</Card.Description>
						</div>
					</Card.Header>
				</Card.Root>
			</Reveal>
			...
			<Reveal delay={600} x={-20}>
				<Card.Root class="rounded-none border-primary/5 bg-muted/20 hover:bg-muted/30 transition-colors">
					<Card.Header class="flex flex-row items-center gap-4 space-y-0">
						...
						<div>
							<Card.Title class="text-xs font-black uppercase tracking-widest text-muted-foreground">Phone</Card.Title>
			...
			<Reveal delay={700} x={-20}>
				<Card.Root class="rounded-none border-primary/5 bg-muted/20 hover:bg-muted/30 transition-colors">
					<Card.Header class="flex flex-row items-center gap-4 space-y-0">
						...
						<div>
							<Card.Title class="text-xs font-black uppercase tracking-widest text-muted-foreground">Location</Card.Title>
							<Card.Description class="text-lg font-bold text-foreground">Jakarta/Palembang, Indonesia</Card.Description>
						</div>
					</Card.Header>
				</Card.Root>
			</Reveal>
		...
		<div class="lg:col-span-3">
			<Reveal delay={500} x={20}>
				<Card.Root class="rounded-none border-primary/10 shadow-2xl shadow-primary/5 overflow-hidden">
					<Card.Content class="p-8">
						{#if status === 'success'}
							<div class="py-12 flex flex-col items-center text-center space-y-4" in:fade>
								<div class="size-20 rounded-none bg-green-500/10 flex items-center justify-center text-green-500">
									<CheckCircle2 class="size-12" />
								</div>
								<h2 class="text-2xl font-black uppercase tracking-tight">{m.form_success()}</h2>
								<Button variant="outline" onclick={() => status = 'idle'} class="rounded-none font-bold uppercase tracking-widest text-xs">
									{m.form_reset()}
								</Button>
							</div>
						{:else}
							<form onsubmit={handleSubmit} class="space-y-6">
								<div class="space-y-2">
									<Label for="name" class="text-xs font-black uppercase tracking-widest">{m.form_name()}</Label>
									<Input 
										id="name" 
								...
								<div class="space-y-2">
									<Label for="email" class="text-xs font-black uppercase tracking-widest">{m.form_email()}</Label>
									<Input 
										id="email" 
										type="email" 
								...
								<div class="space-y-2">
									<Label for="message" class="text-xs font-black uppercase tracking-widest">{m.form_message()}</Label>
									<Textarea 
										id="message" 
